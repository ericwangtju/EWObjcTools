...
EWSYNTH_DUMMY_CLASS(UIDevice_EWAdd)

@implementation UIDevice (EWSugar)
...
typedef struct {
    ...
    uint64_t awdl_in;
    uint64_t awdl_out;
} ew_net_interface_counter;


static uint64_t ew_net_counter_add(uint64_t counter, uint64_t bytes) {
...
static uint64_t ew_net_counter_get_by_type(ew_net_interface_counter *counter, EWNetworkTrafficType type) {
    uint64_t bytes = 0;
    if (type & EWNetworkTrafficTypeWWANSent) bytes += counter->pdp_ip_out;
    if (type & EWNetworkTrafficTypeWWANReceived) bytes += counter->pdp_ip_in;
    if (type & EWNetworkTrafficTypeWIFISent) bytes += counter->en_out;
    if (type & EWNetworkTrafficTypeWIFIReceived) bytes += counter->en_in;
    if (type & EWNetworkTrafficTypeAWDLSent) bytes += counter->awdl_out;
    if (type & EWNetworkTrafficTypeAWDLReceived) bytes += counter->awdl_in;
    return bytes;
}

static ew_net_interface_counter ew_get_net_interface_counter() {
    ...
    ew_net_interface_counter counter = {0};
        ...
        while (cursor) {
            if (cursor->ifa_addr->sa_family == AF_LINK) {
                ...
                if (name) {
                    ...
                    counter_in = ew_net_counter_add(counter_in, data->ifi_ibytes);
                    sharedInCounters[name] = @(counter_in);
                    
                    uint64_t counter_out = ((NSNumber *)sharedOutCounters[name]).unsignedLongLongValue;
                    counter_out = ew_net_counter_add(counter_out, data->ifi_obytes);
...
- (uint64_t)getNetworkTrafficBytes:(EWNetworkTrafficType)types {
    ew_net_interface_counter counter = ew_get_net_interface_counter();
    return ew_net_counter_get_by_type(&counter, types);
}
