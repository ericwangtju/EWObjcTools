...
EWSYNTH_DUMMY_CLASS(UIColor_EWSugar)

#define CLAMP_COLOR_VALUE(v) (v) = (v) < 0 ? 0 : (v) > 1 ? 1 : (v)

void EW_RGB2HSL(CGFloat r, CGFloat g, CGFloat b,
...
void EW_HSL2RGB(CGFloat h, CGFloat s, CGFloat l,
...
void EW_RGB2HSB(CGFloat r, CGFloat g, CGFloat b,
...
void EW_HSB2RGB(CGFloat h, CGFloat s, CGFloat v,
                CGFloat *r, CGFloat *g, CGFloat *b) {
    CLAMP_COLOR_VALUE(h);
    CLAMP_COLOR_VALUE(s);
    CLAMP_COLOR_VALUE(v);
...
void EW_RGB2CMYK(CGFloat r, CGFloat g, CGFloat b,
...
void EW_CMYK2RGB(CGFloat c, CGFloat m, CGFloat y, CGFloat k,
                 CGFloat *r, CGFloat *g, CGFloat *b) {
    CLAMP_COLOR_VALUE(c);
    CLAMP_COLOR_VALUE(m);
    CLAMP_COLOR_VALUE(y);
...
void EW_HSB2HSL(CGFloat h, CGFloat s, CGFloat b,
...
void EW_HSL2HSB(CGFloat h, CGFloat s, CGFloat l,
...
@implementation UIColor (EWSugar)
...
+ (UIColor *)colorWithHue:(CGFloat)hue
               saturation:(CGFloat)saturation
                lightness:(CGFloat)lightness
                    alpha:(CGFloat)alpha {
    CGFloat r, g, b;
    EW_HSL2RGB(hue, saturation, lightness, &r, &g, &b);
...
+ (UIColor *)colorWithCyan:(CGFloat)cyan
                   magenta:(CGFloat)magenta
                    yellow:(CGFloat)yellow
                     ...
                     alpha:(CGFloat)alpha {
    CGFloat r, g, b;
    EW_CMYK2RGB(cyan, magenta, yellow, black, &r, &g, &b);
    return [UIColor colorWithRed:r green:g blue:b alpha:alpha];
}
...
- (BOOL)getHue:(CGFloat *)hue
    ...
    EW_RGB2HSL(r, g, b, hue, saturation, lightness);
    *alpha = a;
    return YES;
}
...
- (BOOL)getCyan:(CGFloat *)cyan
    ...
    EW_RGB2CMYK(r, g, b, cyan, magenta, yellow, black);
    *alpha = a;
    return YES;
}
...
+ (UIColor *)colorWithColorString:(NSString *)color alpha:(CGFloat)alpha {
  ...
  // 删除字符串中的空格
  NSString *oString = [[color stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]] uppercaseString];
  ...
  // 如果0x开头的，那么截取字符串，字符串从索引为2的位置开始，一直到末尾
  if ([oString hasPrefix:@"0X"]) {
    
    oString = [oString substringFromIndex:2];
  }
  
  // 如果是#开头的，那么截取字符串，字符串从索引1的位置开始，一直到末尾
  if ([oString hasPrefix:@"#"]) {
    
    oString = [oString substringFromIndex:1];
  }
  ...
  // r
  NSString *rString = [oString substringWithRange:range];
  // g
  NSString *gString = [oString substringWithRange:range];
  // b
  NSString *bString = [oString substringWithRange:range];
