...
#import "UIApplication+EWSugar.h"
#import "NSArray+EWSugar.h"
#import "NSObject+EWSugar.h"
...
EWSYNTH_DUMMY_CLASS(UIApplication_EWSugar)

#define kNetworkIndicatorDelay (1/30.0)
@interface _EWUIApplicationNetworkIndicatorInfo : NSObject
@property (nonatomic, assign) NSInteger count;
@property (nonatomic, strong) NSTimer *timer;
@end

@implementation _EWUIApplicationNetworkIndicatorInfo
@end

@implementation UIApplication (EWSugar)
- (NSURL *)documentsURL {
...
- (BOOL)isPirated {
    ...
    if (![self _ew_fileExistInMainBundle:@"_CodeSignature"]) {
        return YES;
    }
    
    if (![self _ew_fileExistInMainBundle:@"SC_Info"]) {
        return YES;
    }
...
- (BOOL)_ew_fileExistInMainBundle:(NSString *)name {
...
EWSYNTH_DYNAMIC_PROPERTY_OBJECT(networkActivityInfo, setNetworkActivityInfo, RETAIN_NONATOMIC, _EWUIApplicationNetworkIndicatorInfo *);
...
- (void)_changeNetworkActivityCount:(NSInteger)delta {
    @synchronized(self){
        dispatch_async_on_main_queue(^{
            _EWUIApplicationNetworkIndicatorInfo *info = [self networkActivityInfo];
            if (!info) {
                info = [_EWUIApplicationNetworkIndicatorInfo new];
