...
CGContextRef EWCGContextCreateARGBBitmapContext(CGSize size, BOOL opaque, CGFloat scale) {
...
CGContextRef EWCGContextCreateGrayBitmapContext(CGSize size, CGFloat scale) {
    size_t width = ceil(size.width * scale);
    size_t height = ceil(size.height * scale);
    if (width < 1 || height < 1) return NULL;
...
CGAffineTransform EWCGAffineTransformGetFromViews(UIView *from, UIView *to) {
    ...
    return EWCGAffineTransformGetFromPoints(before, after);
}
